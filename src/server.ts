...
import { getCachedActiveListings, invalidateActiveListings } from '../Redis/activeListingsCache.ts';
import { getCachedChatMessages, appendCachedMessage } from '../Redis/chatMessagesCache.ts';
import { getCachedTopListings, invalidateTopListings } from '../Redis/topListingsCache.ts';
import { getCachedInterestedUsers, addInterestedUser, isUserInterested } from '../Redis/listingInterestedUsersCache.ts';
import { getCachedUserChats, invalidateUserChats } from '../Redis/userChatsCache.ts';
import {
    getAllUsers,
    getUserById,
    createUser,
...
app.get('/api/listings/active', async (_req, res) => {
    const result = await getCachedActiveListings();
    if (result.success) {
        res.json(result.data);
    } else {
        res.status(500).json({ error: result.error });
...
app.get('/api/listings/top', async (_req, res) => {
    try {
        const listings = await getCachedTopListings(10);
        res.json(listings);
    } catch (e) {
...
app.post('/api/interests', async (req, res) => {
    ...
    const alreadyApplied = await isUserInterested(listingId, renterId);
    if (alreadyApplied) {
        return res.status(409).json({ error: 'Already expressed interest in this listing' });
    }
    const result = await createUserInterest(renterId, listingId);
    if (result.success) {
        await Promise.all([
            invalidateActiveListings(),
            invalidateTopListings(),
            addInterestedUser(listingId, renterId),
        ]);
        res.json(result.data);
    } else {
        res.status(500).json({ error: result.error });
...
app.get('/api/listings/:id/interested-users', async (req, res) => {
    try {
        const renterIds = await getCachedInterestedUsers(req.params.id);
        res.json(renterIds);
    } catch (e) {
        res.status(500).json({ error: (e as Error).message });
    }
...
app.get('/api/users/:id/chats', async (req, res) => {
    const result = await getCachedUserChats(req.params.id);
    if (result.success) {
        res.json(result.data);
    } else {
        res.status(500).json({ error: result.error });
...
app.get('/api/messages/chat', async (req, res) => {
    ...
    try {
        const messages = await getCachedChatMessages(from, to);
        res.json(messages);
    } catch (e) {
        res.status(500).json({ error: (e as Error).message });
    }
...
app.post('/api/messages', async (req, res) => {
    ...
    if (result.success) {
        const { sender_id, receiver_id, content, sent_at, read } = req.body ?? {};
        await Promise.all([
            appendCachedMessage(sender_id, receiver_id, {
                message_id: '',
                sender_id,
                receiver_id,
                content,
            ...
            invalidateUserChats(sender_id, receiver_id),
        ]);
        res.json(result.data);
    } else {
        res.status(500).json({ error: result.error });
